...
function parseArgs(argv) {
  const args = {
    ...
    token: process.env.MEDIA_ADMIN_TOKEN || "",
    open: true,
  };
...
function printHelp() {
  console.log(`Start a local/server GUI for Bilibili MV import.

Usage:
  pnpm bili:video-gui
  MEDIA_ADMIN_TOKEN=secret node scripts/bili-video-gui.mjs --port 3200 --no-open

Options:
      --host <host>   Bind host. Default: 127.0.0.1
  -p, --port <port>   Bind port. Default: 3200
      --token <token> Require Bearer token for job APIs.
...
function appendLog(job, text) {
  job.log += text;
  if (job.log.length > 300000) {
    job.log = job.log.slice(-240000);
  }
}
...
async function createJob(payload) {
  ...
  const tempDir = path.join(os.tmpdir(), `fallrain-bili-video-${id}`);
  const inputFile = path.join(tempDir, "bili-mv-list.txt");
  ...
  const script = path.join(cwd, "scripts", "download-bili-video.mjs");
  const childArgs = [
    script,
    "--input",
    inputFile,
    "--output",
    optionalString(payload.output) || "/opt/mv-library/source",
    "--category",
    optionalString(payload.category) || "Bilibili",
    "--quality",
    optionalString(payload.quality) || "best",
    "--merge-format",
    optionalString(payload.mergeFormat) || "mp4",
  ];

  if (payload.formatSelector) {
    childArgs.push("--format-selector", optionalString(payload.formatSelector));
  }
  if (payload.ytDlp) {
    childArgs.push("--yt-dlp", optionalString(payload.ytDlp));
  }
  ...
  if (payload.generate !== false) {
    childArgs.push(
      "--generate",
      "--public-output",
      optionalString(payload.publicOutput) || "/opt/mv-library/public",
      "--public-base",
      optionalString(payload.publicBase) || "/mv-library",
      "--category-depth",
      String(
  ...
  if (payload.noWriteMetadata) childArgs.push("--no-write-metadata");
  if (payload.cookies)
  ...
  <body>
    <main>
      <header>
        <div>
          <h1>MV 下载控制台</h1>
          <p class="hint">输入 BV 号或 Bilibili 视频链接，服务器执行下载、合并 mp4，并生成独立的 mv.json。</p>
        </div>
      ...
      <div class="grid">
        <section>
          ...
          <input id="token" type="password" placeholder="服务器设置了 MEDIA_ADMIN_TOKEN 时填写" />
          <p class="small">Token 只保存在当前浏览器 localStorage，用于调用本机/服务器任务 API。</p>

          <label for="items" style="margin-top:14px">BV / URL 列表</label>
          <textarea id="items" placeholder="BV1BB4y1k7o1&#10;https://www.bilibili.com/video/BVxxxxxxxxxx"></textarea>

          <div class="row">
            <div>
              <label for="category">分类</label>
              <input id="category" value="rock" />
            </div>
            <div>
              <label for="quality">清晰度</label>
              <select id="quality">
                <option value="best" selected>最高可用</option>
                <option value="2160">4K / 2160p 以内</option>
                <option value="1440">2K / 1440p 以内</option>
                <option value="1080">1080p 以内</option>
                <option value="720">720p 以内</option>
                <option value="480">480p 以内</option>
                <option value="360">360p 以内</option>
              </select>
            </div>
          </div>
          <div class="row">
            <div>
              <label for="mergeFormat">视频容器</label>
            ...
            <div>
              <label for="output">MV 源目录</label>
              <input id="output" value="/opt/mv-library/source" />
            </div>
          </div>
          <div class="row">
            <div>
              <label for="publicOutput">生成目录</label>
              <input id="publicOutput" value="/opt/mv-library/public" />
            </div>
          </div>
          <div class="row">
            <div>
              <label for="publicBase">Public Base</label>
              <input id="publicBase" value="/mv-library" />
            </div>
            <div>
          ...
          <div class="row">
            <div>
              <label for="formatSelector">yt-dlp 格式选择器</label>
              <input id="formatSelector" placeholder="高级选项：留空则使用上方清晰度" />
            </div>
            <div>
          ...
          <div class="row">
            <div>
              ...
              <input id="cookiesFromBrowser" placeholder="服务器一般留空；本地可填 edge/chrome" />
            </div>
            <div>
              <label for="cookies">Cookies 文件</label>
              <input id="cookies" placeholder="/root/cookies.txt，可留空" />
            </div>
          </div>
          <div class="row">
            <div>
            ...
            <div>
              <label for="ytDlp">yt-dlp 路径</label>
              <input id="ytDlp" placeholder="/usr/local/bin/yt-dlp，可留空" />
            </div>
          </div>
          <div class="row">
            <div>
          ...
          <div class="check-row">
            <label class="check"><input id="generate" type="checkbox" checked />生成 mv.json</label>
            ...
            <label class="check"><input id="noWriteMetadata" type="checkbox" />不写元数据</label>
          </div>
          <div class="actions">
            <button id="start">开始下载</button>
            <button class="secondary" id="clear" type="button">清空日志</button>
          </div>
          <p class="small">服务器部署建议保持监听 127.0.0.1，再通过 Nginx 受保护路径访问。不要公开无鉴权入口。</p>
        </section>
        <section>
          <label>执行日志</label>
          <pre id="log"></pre>
    ...
    <script>
      ...
      $("token").value = localStorage.getItem("fallrain-mv-admin-token") || "";
      $("token").addEventListener("change", () => {
        localStorage.setItem("fallrain-mv-admin-token", $("token").value);
      });
      ...
      function readPayload() {
        return {
          ...
          noWriteMetadata: $("noWriteMetadata").checked,
        };
      }
      ...
      async function poll() {
        ...
        if (!res.ok) throw new Error(job.error || "读取任务失败");
      ...
      async function readJsonResponse(res) {
        ...
        if (!text.trim()) {
          throw new Error("服务器返回了空响应，请检查 Nginx 反代路径。");
        }
        try {
          return JSON.parse(text);
        } catch {
          throw new Error("服务器返回的不是 JSON，请检查 /media-admin/ 反代是否指向 127.0.0.1:3200。");
        }
      }
    </script>
  </body>
...
async function handleRequest(req, res, args) {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/") && !isAuthorized(req, args.token)) {
...
const server = createServer((req, res) => {
  handleRequest(req, res, args);
});

server.listen(args.port, args.host, () => {
  const url = `http://${args.host}:${args.port}`;
  console.log(`Bilibili MV GUI: ${url}`);
  console.log(
    args.token
      ? "Job APIs require Bearer token."
      : "Job APIs are not token-protected.",
  );
